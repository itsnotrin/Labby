...
final class ProxmoxDetailViewModel: ObservableObject {
    ...
    func refresh() async {
        ...
        do {
            ...
            await MainActor.run {
                self.isUsingCachedData = !dataChanged && beforeVMs > 0
                self.lastCacheUpdate = Date()
            }
...
// MARK: - Sub-Views (Stubs for detailed views)
...
struct ProxmoxVMsView: View {
    ...
    private func refreshData() async {
        isRefreshing = true
        // Allow parent view to handle refresh
    ...
    var body: some View {
        ScrollView {
            LazyVStack(spacing: 16) {
                ...
                VMStatusSection(vm: vm, viewModel: viewModel)

                // Resource Usage
                VMResourceSection(vm: vm)
                ...
                // Management Actions
                VMManagementSection(vm: vm, viewModel: viewModel)
